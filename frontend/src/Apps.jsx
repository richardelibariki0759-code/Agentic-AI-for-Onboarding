...
const TOPICS = [
  { label: "ONA App", icon: "📱", desc: "Forms, data collection & sync" },
  { label: "Data Collection", icon: "🌿", desc: "Field methods & image capture" },
  { label: "Bruno Pushcart", icon: "🤖", desc: "Device assembly & setup" },
];
...
function splitAtCheckpoint(content) {
  ...
  // Try to split at the --- divider that precedes the checkpoint
  ...
  // Fallback: split at ✅ directly (with or without ** bold markers)
...
function Message({ msg }) {
  ...
  // 1. Extract any inline [Image URL: ...] or bare URLs embedded in the text
  const { cleaned: cleanedContent, urls: inlineUrls } = isBot
    ? extractAndCleanImages(msg.content)
    : { cleaned: msg.content, urls: [] };

  // 2. Merge with images already extracted by the backend (deduplicated)
  const allImageUrls = isBot
    ? [...new Set([...inlineUrls, ...(msg.images || [])])]
    : [];

  // 3. Split cleaned content at the ✅ checkpoint
    ...
    <div style={{ display: "flex", justifyContent: isBot ? "flex-start" : "flex-end", marginBottom: 16, animation: "fadeSlideIn 0.25s ease" }}>
      ...
      <div style={{ maxWidth: "75%" }}>
        ...
        <div style={{
          ...
          {/* Images displayed BEFORE the ✅ checkpoint question */}
          {isBot && allImageUrls.length > 0 && (
            <ImageGallery urls={allImageUrls} />
          )}

          {/* The ✅ checkpoint line (and anything after) — also cleaned */}
...
function AdminUploadPanel() {
  ...
  return (
    <div style={{
      ...
      {/* File picker — filename shown, no URL */}
      <div
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "48px 24px 24px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div style={{ textAlign: "center", maxWidth: 520, marginBottom: 48 }}>
        ...
        <p style={{ fontSize: 16, color: "var(--color-text-secondary)", margin: 0, lineHeight: 1.6 }}>
          Your AI training assistant for ONA, Bruno & data collection workflows.
        </p>
      </div>
...
export default function App() {
  ...
  // Admin mode: read from URL param ?admin=1 (or set via env/config)
  // Users never see the upload UI unless this is true.
